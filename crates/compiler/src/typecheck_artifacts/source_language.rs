...
const SERVER_SURFACE: &str = r#"{
  ...
  "serverConfigModules": ["any imported .dowe module", "classified by database, cache, vector, or queue declaration", "database appDb provider:\"dowe\" host:env.DB_HOST port:env.DB_PORT account:env.DB_USER secret:env.DB_PASSWORD name:\"app\" entities:[Users] seeders:[Bootstrap]", "cache appCache provider:\"dowe\" host:env.CACHE_HOST port:env.CACHE_PORT account:env.CACHE_USER secret:env.CACHE_PASSWORD name:env.CACHE_NAME", "vector appVector provider:\"dowe\" host:env.VECTOR_HOST port:env.VECTOR_PORT account:env.VECTOR_USER secret:env.VECTOR_PASSWORD name:env.VECTOR_DATABASE", "queue appQueue provider:\"dowe|rabbitmq\" host:env.QUEUE_HOST port:env.QUEUE_PORT account:env.QUEUE_USER secret:env.QUEUE_PASSWORD vhost:env.QUEUE_VHOST"],
