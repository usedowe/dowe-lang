...
const SERVER_SURFACE: &str = r#"{
  "root": "main.dowe",
  "blocks": ["type", "entity", "seeder", "database", "cache", "vector", "main", "app", "views", "server", "tls", "desktop", "endpoints", "group", "get", "post", "put", "delete", "patch", "route", "websocket", "udp", "tcp", "packet", "connection", "rtp", "model", "init", "handler", "middleware", "fn"],
  "httpMethods": ["get", "post", "put", "delete", "patch"],
  "actions": ["functionName result args:{ ... }", "const binding value:req.json", "namespace result source:\"stdlibFunction\"", "request result source:\"query|rawQuery|header|cookie\"", "ws result source:\"json\"", "agent result source:\"chat\" request:request", "session result cache:cache database:database token:token", "return status:201 json:value", "return value", "next", "response text", "response json", "send ws", "bridge sse", "go functionName args:{ ... }", "cron functionName schedule:\"0 * * * *\" args:{ ... }", "log", "info", "warn", "error"],
  "request": ["req.params", "req.json", "req.context", "request result source:\"query\"", "request result source:\"rawQuery\"", "request result source:\"header\" name:\"Range\"", "request result source:\"cookie\" name:\"session\"", "ws result source:\"json\""],
  ...
  "backgroundJobs": ["go", "cron", "isolated process", "fire-and-forget", "UTC five-field schedule", "static JSON args"],
  "serverConfigModules": ["any imported .dowe module", "classified by database, cache, or vector declaration", "database appDb provider:\"dowe\" host:env.DB_HOST port:env.DB_PORT account:env.DB_USER secret:env.DB_PASSWORD name:\"app\" entities:[Users] seeders:[Bootstrap]", "cache appCache provider:\"dowe\" host:env.CACHE_HOST port:env.CACHE_PORT account:env.CACHE_USER secret:env.CACHE_PASSWORD name:env.CACHE_NAME", "vector appVector provider:\"dowe\" host:env.VECTOR_HOST port:env.VECTOR_PORT account:env.VECTOR_USER secret:env.VECTOR_PASSWORD name:env.VECTOR_DATABASE"],
  "tls": ["tls mode:\"acme\" domains:[\"example.com\"] email:\"admin@example.com\" staging:false", "tls mode:\"local\" domains:[\"localhost\"]", "domainsFrom KV or Database", "automatic ACME renewal", "server only"],
  "jwt": {
    "jws": ["HS256"],
    "jwe": ["dir", "A256GCM"],
    "secrets": "server env variables only"
  },
  "inferredReferences": ["store.insert field", "store.insert id", "store.update changed", "store.delete changed", "kv.set ok", "kv.set key", "kv.delete deleted", "kv.clear cleared", "emb.upsert id", "emb.upsert dimensions", "emb.upsert created", "emb.delete deleted"],
...
const VIEWS_SURFACE: &str = r#"{
  ...
  "startup": ["one direct init per layout or page", "init uses ordered view function statements", "one direct Splash bind:booleanSignal", "Splash true shows its children", "Splash false shows normal roots", "layout keeps one normal root", "page keeps one or more normal roots", "web Android iOS"],
  ...
  "sizing": ["w", "h", "minW", "minH", "maxW", "maxH", "responsive values", "h minH maxH accept vh-<scale>"],
  ...
  "reactiveProps": {"Button": {"stringSignals": ["variant", "scheme", "size", "rounded"], "booleanSignals": ["loading"], "conditionalIcons": {"props": ["iconStart", "iconEnd"], "boolean": "when:path", "numeric": ["gt", "gte", "lt", "lte"]}}, "SideNav": {"stringSignals": ["variant", "scheme", "size"], "booleanSignals": ["wide"]}, "Code": {"template": "validated {signal.path} placeholders in static content"}},
