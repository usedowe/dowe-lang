use dowe_agent::native_harness::{
    HarnessStore, MAX_EXTENSION_CALLS, SessionExtensionCall, SessionObserver,
};
use serde_json::json;
...
fn registry() -> (
    TempDir,
    TempDir,
    dowe_agent::native_harness::SessionExtensionRegistry,
    String,
) {
    ...
    session
        .events
        .push(json!({"event":"response", "prompt":"secret", "secret":"secret"}));
...
fn exposes_deterministic_fixed_read_only_registry_and_redacted_events() {
    ...
    assert_eq!(
        ...
        dowe_agent::native_harness::session_extension_descriptor_fingerprint()
    );
    let result = registry
        .call(SessionExtensionCall {
            session_id,
...
fn rejects_foreign_unknown_malformed_and_closed_calls() {
    ...
    let call = |id: &str, args| {
        registry.call(SessionExtensionCall {
...
fn enforces_call_bound() {
    ...
    for _ in 0..MAX_EXTENSION_CALLS {
        let _ = registry.call(SessionExtensionCall {
            session_id: session_id.clone(),
            tool_id: "events.poll".into(),
            arguments: json!({"since": 0}),
        });
