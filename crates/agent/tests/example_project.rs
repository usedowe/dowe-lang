...
#[test]
fn compiles_the_initial_marketing_blocks_example() {
    let _guard = ENV_LOCK.lock().expect("env lock");
    let temp = TempDir::new().expect("tempdir");
    let source = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/embedded/examples/marketing-ui");
    copy_tree(&source, temp.path());
    fs::copy(temp.path().join(".env.example"), temp.path().join(".env")).expect("env");
    let quality = audit_dowe_project(temp.path()).expect("marketing UI quality audit");
    assert_eq!(quality["status"], "passed", "{quality}");
    ...
    result.expect("marketing blocks example");
}
