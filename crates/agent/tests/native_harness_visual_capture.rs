...
fn screenshot_tool_is_execute_only_and_rejects_non_loopback_urls() {
    ...
    assert!(tools.prepare(&ToolCall::new("x", "capture_web_screenshot", json!({
        "url": "https://example.com",
        "reason": "inspect rendered page"
    })), HarnessRole::Execute).is_err());
    assert!(tools.prepare(&ToolCall::new("x", "capture_web_screenshot", json!({
        "url": "http://127.0.0.1:3000",
        "reason": "inspect rendered page"
    })), HarnessRole::Plan).is_err());
}
...
fn capture_call() -> ToolCall {
    ToolCall::new("x", "capture_web_screenshot", json!({"url":"http://localhost:3000/app","reason":"inspect rendered page"}))
}
...
fn fake_browser_does_not_receive_standalone_profile_argument() {
    ...
    let browser = fake_browser(root.path(), &format!("#!/bin/sh\nfor arg in \"$@\"; do printf '%s\\n' \"$arg\" >> '{}'; done\nout=\"\"; for arg in \"$@\"; do case \"$arg\" in --screenshot=*) out=${{arg#--screenshot=}};; esac; done; mkdir -p \"$(dirname \"$out\")\"; {}\n", argv.display(), VALID_PNG));
    unsafe { std::env::set_var("DOWE_AGENT_BROWSER", &browser); }
    let mut tools = HarnessTools::new(root.path(), "session-argv", HarnessConfig::default()).unwrap();
    let approval = tools.prepare(&capture_call(), HarnessRole::Execute).unwrap().unwrap();
    tools.capture_web_screenshot(approval).unwrap();
    let args = std::fs::read_to_string(argv).unwrap();
    assert!(!args.lines().any(|arg| arg.starts_with(".profile-") || arg.ends_with("/.profile-")));
    unsafe { std::env::remove_var("DOWE_AGENT_BROWSER"); }
}
...
fn symlinked_visual_qa_ancestor_is_rejected() {
    ...
    unsafe { std::env::set_var("DOWE_AGENT_BROWSER", &browser); }
    let mut tools = HarnessTools::new(root.path(), "session-link", HarnessConfig::default()).unwrap();
    let approval = tools.prepare(&capture_call(), HarnessRole::Execute).unwrap().unwrap();
...
fn fake_browser_captures_to_isolated_visual_qa_session() {
    ...
    assert!(root.path().join(".dowe/visual-qa/session-1/rendered.png").is_file());
    unsafe { std::env::remove_var("DOWE_AGENT_BROWSER"); }
}
