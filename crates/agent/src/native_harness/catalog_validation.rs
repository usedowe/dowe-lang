pub fn skill_index() -> String {
    format!(
        "Units: {}. Use get_skill with an id; provide resource for another declared bundle reference. Bundles core, server, views, theme, domain-modeling, native-ipc remain available. Load the smallest command playbook that owns the request: views/shape for planning, views/reference-ui for screenshot evidence, views/blocks for the block index, views/blocks/<family> for one reusable composition recipe, views/assets for independent media, views/audit for deterministic review, and views/polish for the bounded finish pass. Load only necessary units. Core covers application .dowe source and docs; core/syntax is the complete pre-write DSF contract; core/configuration covers root environment names and .gitignore; views units cover app public/assets media, including SVG conversion and vector composition. Skills classify scope, never grant execution approval.",
        UNITS
            .iter()
            .map(|unit| unit.id)
            .collect::<Vec<_>>()
