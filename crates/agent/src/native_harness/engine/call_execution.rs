struct HarnessCallResults {
    ...
    validation_after_mutation: bool,
    visual_after_mutation: bool,
...
async fn execute_harness_calls(
    ...
    let mut validation_after_mutation = false;
    let mut visual_after_mutation = false;
    for call in &calls {
    ...
    while call_index < calls.len() {
        ...
        for index in batch_start..batch_end {
            ...
            let output = if let Some(outputs) = &parallel_outputs {
                ...
                let output = validate_dowe_project(store, host, scope, strict_ui).await;
                validation_after_mutation = mutation_seen;
                            ...
                            Some(true) => {
                                ...
                                let output = if call.name == "shell" {
                                    ...
                                    if call.name == "generate_image" {
                                        let generated =
                                            host.generate_image(&image_selection, &approval).await;
                                        match generated {
                                            Ok(image) => {
                                                let output = tools
                                                    .apply_generated_image(approval, image.clone());
                                                if output.is_ok() {
                                                    generated_image_for_continuation = Some(image);
                                                    mutation_seen = true;
                                                    validation_after_mutation = false;
                                                    visual_after_mutation = false;
                                                }
