...
impl SessionExtensionRegistry {
    ...
    pub fn descriptors(&self) -> Vec<SessionExtensionToolDescriptor> {
        fixed_descriptors()
    }
    pub fn fingerprint(&self) -> String {
        descriptor_fingerprint(&fixed_descriptors())
    }
    ...
    pub fn call(&self, request: SessionExtensionCall) -> AgentResult<SessionExtensionResult> {
        if self.closed.load(Ordering::Acquire) {
            return Err(AgentError::new("extension registry is closed"));
        }
        if request.session_id != self.observer.session_id() {
            return Err(AgentError::new("extension call belongs to another session"));
        }
        let encoded = serde_json::to_vec(&request.arguments)?;
        if encoded.len() > MAX_EXTENSION_REQUEST_BYTES {
            return Err(AgentError::new("extension request exceeds the byte limit"));
        }
        let call = self.calls.fetch_add(1, Ordering::AcqRel) + 1;
        if call > MAX_EXTENSION_CALLS {
            return Err(AgentError::new("extension call limit exceeded"));
        }
        if !self.handle.is_active() {
            return Err(AgentError::new("observer is inactive or unsubscribed"));
        }
        let descriptor = fixed_descriptors()
            .into_iter()
            .find(|tool| tool.id == request.tool_id)
            .ok_or_else(|| AgentError::new("unknown session extension tool"))?;
        if encoded.len() > descriptor.input_limit {
            return Err(AgentError::new(
                "extension arguments exceed the tool input limit",
            ));
        }
        ...
        if output_bytes.len() > descriptor.output_limit
            || output_bytes.len() > MAX_EXTENSION_RESPONSE_BYTES
        {
            return Err(AgentError::new("extension response exceeds the byte limit"));
        }
        Ok(SessionExtensionResult {
...
impl SessionObserver {
    pub fn create_extension_registry(
        &self,
        handle: SessionObserverHandle,
    ) -> AgentResult<SessionExtensionRegistry> {
...
fn descriptor(
    ...
    input_limit: usize,
    output_limit: usize,
) -> SessionExtensionToolDescriptor {
    SessionExtensionToolDescriptor {
        id: id.into(),
...
pub fn validate_session_extension_descriptors(
    descriptors: &[SessionExtensionToolDescriptor],
) -> AgentResult<()> {
    if descriptors.is_empty() || descriptors.len() > MAX_EXTENSION_TOOLS {
        return Err(AgentError::new(
            "extension registration count is outside the allowed range",
        ));
    }
    ...
    for descriptor in descriptors {
        if descriptor.id.is_empty() || !ids.insert(&descriptor.id) {
            return Err(AgentError::new(
                "extension tool IDs must be unique and non-empty",
            ));
        }
        if descriptor.version.is_empty()
            || descriptor.capability.is_empty()
            || descriptor.input_limit == 0
            || descriptor.output_limit == 0
        {
            return Err(AgentError::new("extension descriptor is invalid"));
        }
    }
    Ok(())
}
fn descriptor_fingerprint(descriptors: &[SessionExtensionToolDescriptor]) -> String {
    let mut material: Vec<String> = descriptors
        .iter()
        .map(|d| {
            format!(
                "{}\0{}\0{}\0{}\0{}",
                d.id, d.version, d.capability, d.input_limit, d.output_limit
            )
        })
        .collect();
    material.sort();
...
fn poll_events(
    ...
    let object = args
        .as_object()
        .ok_or_else(|| AgentError::new("events.poll arguments must be an object"))?;
    let since = object
        .get("since")
        .and_then(Value::as_u64)
        .ok_or_else(|| AgentError::new("events.poll requires an integer since"))?;
    let limits = SessionEventLimits {
        max_events: object
            .get("maxEvents")
            .and_then(Value::as_u64)
            .unwrap_or(super::MAX_EVENTS as u64) as usize,
        max_bytes: object
            .get("maxBytes")
            .and_then(Value::as_u64)
            .unwrap_or(super::MAX_PAGE_BYTES as u64) as usize,
    };
    Ok(serde_json::to_value(observer.poll(handle, since, limits)?)?)
}

pub fn session_extension_descriptor_fingerprint() -> String {
    descriptor_fingerprint(&fixed_descriptors())
}

pub fn session_extension_fingerprint(
    descriptors: &[SessionExtensionToolDescriptor],
) -> AgentResult<String> {
