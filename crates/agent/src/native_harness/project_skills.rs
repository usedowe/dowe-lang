...
pub fn discover_project_skills(root: impl AsRef<Path>) -> AgentResult<Vec<ProjectSkillSummary>> {
    ...
    if entries.len() > MAX_PROJECT_SKILL_COUNT {
        return Err(AgentError::at_path(
            &skills_dir,
            format!("project skill count exceeds {MAX_PROJECT_SKILL_COUNT}"),
        ));
    }
    ...
    for entry in entries {
        ...
        if !checked_directory(&directory, &root)? {
            return Err(AgentError::at_path(
                &directory,
                "project skill directory must be a regular directory",
            ));
        }
...
fn load_project_skill_from_canonical(
    root: &Path,
    id: &str,
    total: usize,
) -> AgentResult<ProjectSkill> {
    let directory = root.join(".agents").join("skills").join(id);
    if !checked_directory(&directory, root)? {
        return Err(AgentError::at_path(
            &directory,
            "project skill directory is not a regular directory",
        ));
    }
    ...
    if metadata.file_type().is_symlink() || !metadata.is_file() {
        return Err(AgentError::at_path(
            &path,
            "project skill must be a regular, non-symlink file",
        ));
    }
    ...
    if bytes.len() > MAX_PROJECT_SKILL_FILE_BYTES {
        return Err(AgentError::at_path(
            &path,
            format!("file exceeds {MAX_PROJECT_SKILL_FILE_BYTES} byte limit"),
        ));
    }
    if total.saturating_add(bytes.len()) > MAX_PROJECT_SKILL_AGGREGATE_BYTES {
        return Err(AgentError::at_path(
            &path,
            format!("aggregate skill limit is {MAX_PROJECT_SKILL_AGGREGATE_BYTES} bytes"),
        ));
    }
...
fn canonical_project_root(root: &Path) -> AgentResult<PathBuf> {
    let metadata =
        fs::symlink_metadata(root).map_err(|error| AgentError::at_path(root, error.to_string()))?;
...
fn checked_directory(path: &Path, root: &Path) -> AgentResult<bool> {
    ...
    if metadata.file_type().is_symlink() {
        return Err(AgentError::at_path(
            path,
            "symlink ancestors are not allowed",
        ));
    }
    Ok(metadata.is_dir())
}
...
mod tests {
    ...
    fn discovers_sorted_summaries_and_loads_exact_bytes() {
        ...
        assert_eq!(
            load_project_skill(root.path(), "alpha").unwrap().content,
            "alpha"
        );
    ...
    fn rejects_symlinked_skill_directory() {
        ...
        symlink(
            ...
            root.path().join(".agents/skills/link"),
        )
        .unwrap();
