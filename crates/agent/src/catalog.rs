...
use crate::provider::{AgentModelDefinition, provider_exists};
...
pub fn models_with_local_overrides(provider: &str) -> AgentResult<Vec<AgentCatalogModel>> {
    ...
    let contents =
        fs::read_to_string(&path).map_err(|e| AgentError::at_path(&path, e.to_string()))?;
    ...
    if file.schema_version != 1 {
        return Err(AgentError::at_path(
            &path,
            format!(
                "unsupported model catalog override schema version: {}",
                file.schema_version
            ),
        ));
    }
    for unknown in file.providers.keys().filter(|id| !provider_exists(id)) {
        return Err(AgentError::at_path(
            &path,
            format!("unknown provider in model catalog override: `{unknown}`"),
        ));
    }
    let Some(entries) = file.providers.get(provider) else {
        return Ok(result);
    };
    let mut ids = result
        .iter()
    ...
    for entry in entries {
        ...
        if !ids.insert(entry.id.as_str()) {
            return Err(AgentError::at_path(
                &path,
                format!(
                    "duplicate model id in model catalog override: `{}`",
                    entry.id
                ),
            ));
        }
        ...
        if name.trim().is_empty() || name.chars().any(char::is_control) {
            return Err(AgentError::at_path(
                &path,
                "invalid model name in model catalog override",
            ));
        }
        overrides.push(AgentCatalogModel {
            id: entry.id.clone(),
            name: name.to_string(),
        });
    }
...
fn validate_override_id(id: &str) -> Result<(), String> {
    ...
    {
        return Err(format!(
            "invalid model id in model catalog override: `{id}`"
        ));
    }
    Ok(())
}
...
mod tests {
    ...
    fn snapshot_covers_every_provider() {
        for provider in crate::provider::builtin_provider_ids() {
            assert!(!builtin_models(provider).is_empty(), "{provider}");
        }
    }
    #[test]
    fn snapshot_is_deterministically_sorted() {
        let ids = builtin_models("openrouter")
            .iter()
            .map(|m| m.id)
            .collect::<Vec<_>>();
    ...
    fn override_schema_and_ids_are_validated() {
        let file: OverrideFile = serde_json::from_str(
            r#"{"schemaVersion":1,"providers":{"openai":[{"id":"custom/model"}]}}"#,
        )
        .unwrap();
        assert_eq!(file.schema_version, 1);
        assert!(
            serde_json::from_str::<OverrideFile>(r#"{"schemaVersion":2,"providers":{}}"#).is_ok()
        );
