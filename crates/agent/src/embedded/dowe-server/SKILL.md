...
Server source is compiled into Rust-owned runtime behavior. When a function is called from a native View through `main.ipc`, also load the companion `dowe-native-ipc` skill; keep the function contract and security rules here while the host bridge remains thin. Keep routes, input validation, data access and responses explicit.
...
6. Put Database, Cache, Vector, Queue, and notification work in repository or service functions and reuse imported config connection declarations. Queue connections are server-only; `msg ... publish` targets an already-provisioned queue and returns `{ ok, id }`. `notify` persists a bounded, authenticated process or chat intent and returns `{ ok, id, deliveries }`.
