...
`notify` persists a validated process or chat intent for the authenticated user's active installations.
The local runtime stores the outbox under `.dowe/notifications`; production dispatchers claim delivery
leases and send through Web Push, APNs, FCM, WNS, or the opted-in Linux receiver. Keep the route
internal and put references, rather than credentials or full protected messages, in `data`.

```text
notify sent id:saved.id user:body.recipientId category:"chat" title:"New message" body:"You have a new message" route:"/chat/latest" data:{ messageId:saved.id }
```
