...
This is a structural invariant, not a style preference. Before implementation, perform a shell
ownership pass: if a page source contains `Scaffold`, `Sidebar`, `AppBar`, `Footer`, `Drawer`, or
`children`, it is misclassified and must be split into a layout plus page. The route graph must use
one `group` with `layout:<Layout>` and direct child `route` declarations. A page may own auth,
workspace, loading, and empty states, but those states must be sibling `Section` roots; they must
not be implemented as alternate Scaffolds. Session-aware navigation belongs in the layout and may
use `show:<session.path>` without moving the shell into the page.
