...
| `Scaffold` | The single normal layout root. It accepts optional `appBar`, `start`, `end`, `bottomBar`, and `overlays` regions plus required `main`; `boxed:true` centers only the `start`/`main`/`end` body at `96rem` web or `1536` native. `safeAreaTop` and `safeAreaBottom` independently color the native top and bottom system-inset bands on Android and iOS, defaulting to `background`; web accepts them for target parity. |
