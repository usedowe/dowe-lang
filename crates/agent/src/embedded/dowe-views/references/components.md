...
| `SideNav` | Detailed vertical navigation with optional `header`, direct `item`, `divider`, and `submenu` entries. Use it directly or through a static reusable component in `Sidebar body` and `Drawer body`. `submenu open` is initial state; the runtime retains later toggles in session memory across unmount and remount. Put an optional `id` on the `SideNav` root when structurally identical instances need independent memory; `item` entries do not accept `id`. |
...
| `Scaffold` | The single normal layout root. It accepts optional `appBar`, `start`, `end`, `bottomBar`, and `overlays` regions plus required `main`; place a shell `Footer` inside `main` after `children`, and reserve `bottomBar` for `BottomBar`. `boxed:true` centers only the `start`/`main`/`end` body at `96rem` web or `1536` native. `safeAreaTop` and `safeAreaBottom` independently color the native top and bottom system-inset bands on Android and iOS, defaulting to `background`; web accepts them for target parity. |
...
| `Editor` | Bound rich-text or source editor using the supported language and syntax highlighting; `hideToolbar:true` removes the toolbar row without removing keyboard editing; Ctrl/Cmd+Z undoes, Ctrl/Cmd+Shift+Z and Ctrl/Cmd+Y redo, and `onSave` dispatches a named function on Ctrl/Cmd+S. |
...
| `RadioCard` | Bound single choice presented as selectable cards. Compose it from direct `item` entries with `value`, `title`, optional `description`, optional Solar `icon`, and optional `disabled`; use `orientation` for horizontal or vertical card layouts. |
...
| `Tree` | Compact portable hierarchy for file explorers and nested navigation. Bind a root object with `folders`/`files`, a root object with `children`, or an array of nodes; use `bind` for the selected leaf and `onSelect` for an item-scoped action. |
...
`Tree data:<reference>` accepts a file-tree root with `folders` and `files`, a generic root with
`children`, or an array of nodes. Each node requires a stable `id` and `name` or `label`; folders
and branches may contain nested arrays. `defaultOpen` defaults to `true`, `emptyLabel` and
`ariaLabel` provide direct copy, and `bind` stores the selected leaf `path` or `id`. `onSelect`
runs with the original leaf record as `item`. Web, Android Compose, the Android development
launcher, and iOS share disclosure, indentation, active-row, empty-state, and action behavior.
...
| `ChatBox` | Bound conversation with named send and pagination functions plus loading, sending, streaming, and an optional inline action above the composer through `actionLabel`, `actionVisible`, and `onAction`. Assistant messages may add `title`, `questions:[{ id, question, options, required }]`, `assumptions`, and `nextStep`; bounded `options` render as selectable choices and suspend free-form input until the question set is answered. |
