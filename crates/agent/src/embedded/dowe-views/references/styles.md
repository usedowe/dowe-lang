...
| Controls and forms  | `Input`, `Select`, `Button`, `IconButton`, `ToggleTheme`, `SelectTheme`, `Fab`, `Slider`, `Dropzone`, `ComboBox`, `CsvField`, `DragDrop`, `Editor`, `ImageCropper`, `Password`, `Phone`, `Pin`, `Textarea`, `Record`, `ToggleGroup`, `Checkbox`, `Color`, `Date`, `DateRange`, `RadioGroup`, `RadioCard`, `Toggle`                       |
...
A hidden component does not occupy layout space. Reactive `show` components start hidden in web
and desktop markup until the runtime evaluates their Signal, preventing a pre-hydration flash;
Android and iOS evaluate the condition before building the native component subtree. Responsive
objects accept only booleans; a missing `xs` keeps the component visible below the first declared
breakpoint. Conditional objects
