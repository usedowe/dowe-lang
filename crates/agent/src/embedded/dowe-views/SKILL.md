...
8. Create or reuse a layout whenever the reference has shared chrome. AppBar and Footer never
   ...
   **Shell ownership gate:** before editing a page, classify every persistent region as layout-owned
   or page-owned. If a screen contains `Scaffold`, `Sidebar`, `AppBar`, `Footer`, `Drawer`, or
   `children`, stop and move that shell into `views/layouts/<name>.dowe`. The page may contain only
   one or more sibling `Section` roots and its own state, functions, requests, and page content.
   Connect the page through a route `group path:"/..." layout:<Layout>` and a direct child
   `route`; never attach a layout only after the page has already embedded a second Scaffold.
   Authentication does not change this rule: put the session-aware shell in the layout and expose
   its authenticated regions with `show:<session.path>`, while the auth and workspace page states
   remain Sections in the same page or in separate routed pages.
