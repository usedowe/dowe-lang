...
Cache, or Vector. `id <binding> source:"ulid"` and `hash <binding> source:"sha256"` are server-only.
...
| `math.gt`, `math.gte`, `math.lt`, `math.lte` | `left`, `right` | bool or `null` |
...
| `list.filterContainsAny` | `values`, `field`, `needles` | array |
| `list.concat` | `values`, `other` | array |
...
| Function | Required args | Result |
| --- | --- | --- |
| `hash.sha256` | `value` | lowercase hexadecimal SHA-256 text; server-only |
