...
impl<'a> Reader<'a> {
    ...
    pub fn magic(&mut self, expected: &[u8]) -> StoreResult<()> {
        ...
        if actual != expected {
            return Err(StoreError::Corruption(
                "invalid Database file magic".to_string(),
            ));
        }
        Ok(())
    }
    ...
    pub fn string(&mut self) -> StoreResult<String> {
        ...
        String::from_utf8(bytes.to_vec())
            .map_err(|_| StoreError::Corruption("invalid UTF-8 in Database file".to_string()))
    }
    ...
    fn take(&mut self, length: usize) -> StoreResult<&'a [u8]> {
        ...
        if end > self.bytes.len() {
            return Err(StoreError::Corruption(
                "Database file ended before expected data".to_string(),
            ));
        }
    ...
    Ok(match reader.u8()? {
        ...
        value => {
            return Err(StoreError::Corruption(format!(
                "unknown Database value tag {value}"
            )));
        }
    })
}
