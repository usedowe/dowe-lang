...
pub fn validate_database_name(value: &str) -> StoreResult<()> {
    ...
    if value == "_auth" {
        return Err(StoreError::InvalidName(
            "database name `_auth` is reserved for Database authentication".to_string(),
        ));
    }
    Ok(())
}
...
pub fn validate_account_name(value: &str) -> StoreResult<()> {
...
fn validate_name(value: &str, label: &str) -> StoreResult<()> {
        ...
        || value.chars().any(char::is_control)
    {
        return Err(StoreError::InvalidName(format!(
            "{label} name `{value}` is not safe for Database paths"
        )));
    }
    if !value
        .chars()
