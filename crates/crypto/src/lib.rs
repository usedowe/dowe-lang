...
impl Display for CryptoError {
    fn fmt(&self, formatter: &mut Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::InvalidIv => write!(formatter, "invalid iv"),
            Self::InvalidData => write!(formatter, "invalid data"),
...
pub fn aes_128_ctr(data: &[u8], key: &str, iv: &str) -> Result<Vec<u8>, CryptoError> {
    let key = decode_binary(key)?;
    if key.len() != 16 {
        return Err(CryptoError::InvalidKey);
    }
    let iv = decode_binary(iv)?;
    if iv.len() != 16 {
        return Err(CryptoError::InvalidIv);
    }
    let mut output = data.to_vec();
    let mut cipher = Aes128Ctr::new_from_slices(&key, &iv).map_err(|_| CryptoError::InvalidKey)?;
    cipher.apply_keystream(&mut output);
    Ok(output)
}

pub fn cenc_aes_128_ctr(
    data: &[u8],
    key: &str,
    iv: &str,
    ...
    let mut iv16 = [0u8; 16];
    iv16[..iv.len()].copy_from_slice(&iv);
    let mut output = data.to_vec();
    let mut cipher =
        Aes128Ctr::new_from_slices(&key, &iv16).map_err(|_| CryptoError::InvalidKey)?;
    ...
    for &(clear, encrypted) in subsamples {
        ...
        let end = position
            .checked_add(encrypted)
            .ok_or(CryptoError::InvalidData)?;
        if end > output.len() {
            return Err(CryptoError::InvalidData);
        }
        cipher.apply_keystream(&mut output[position..end]);
        position = end;
    }
...
fn decode_binary(value: &str) -> Result<Vec<u8>, CryptoError> {
    if value.len().is_multiple_of(2) && value.bytes().all(|byte| byte.is_ascii_hexdigit()) {
        let mut output = Vec::new();
        let bytes = value.as_bytes();
        let mut index = 0;
        while index < bytes.len() {
            let high = hex_value(bytes[index]).ok_or(CryptoError::InvalidData)?;
            let low = hex_value(bytes[index + 1]).ok_or(CryptoError::InvalidData)?;
