...
fn side_nav_item(label: &str, path: &str) -> SideNavItemProps {
    SideNavItemProps {
        ...
        description: None,
        description_i18n: None,
        status: (label == "Blogs").then(|| "12".to_string()),
        status_i18n: None,
        icon: (label == "Blogs").then(side_nav_icon),
        on_click: None,
        navigation: Some(NavigationAction::Internal {
            path: path.to_string(),
            fragment: None,
