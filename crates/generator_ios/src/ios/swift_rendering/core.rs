...
fn register_swift_chart_consumed_props(
    component: dowe_components::BuiltinComponent,
    context: &SwiftReactiveContext,
) {
    for (prop, field) in [
...
fn swift_form_variant_props(node: &ViewNode) -> Option<&dowe_components::VariantProps> {
    match node {
...
fn render_swift_node_body(
    ...
    match node {
        ...
        | ViewNode::PieChart { .. }
        | ViewNode::Table { .. }
        | ViewNode::Tree { .. } => render_swift_media_data_node(
            node,
            indent,
            output,
            flow,
