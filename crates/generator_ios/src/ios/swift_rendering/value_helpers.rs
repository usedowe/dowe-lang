...
fn swift_visible_text_expression(
    value: &str,
    i18n: Option<&str>,
    context: &SwiftReactiveContext,
) -> String {
...
fn swift_select_options(
    ...
    let description = each
        ...
        .unwrap_or_else(|| "nil".to_string());
    let dynamic = format!(
        "state.rows(\"{collection}\").map {{ row in DoweSelectOption(value: state.text(\"{value}\", item: row.value), label: state.text(\"{label}\", item: row.value), description: {description}) }}"
    );
    if options.is_empty() {
        dynamic
    } else {
