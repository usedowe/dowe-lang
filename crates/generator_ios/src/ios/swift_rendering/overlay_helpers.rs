...
fn render_swift_dropdown(
    ...
    output.push_str(&format!("{pad}}} content: {{ close in\n"));
    render_swift_region_children(
        header,
        indent + 4,
        output,
    ...
    for entry in entries {
        render_swift_overlay_entry(entry, indent + 4, output, props, context, Some("close"));
    }
    render_swift_region_children(
        footer,
        indent + 4,
...
fn swift_closing_action(action: String, close: &str) -> String {
    if action == "nil" {
        return action;
    }
    let body = action
