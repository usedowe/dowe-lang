...
fn compose_select_options(
    ...
    let values = options
        ...
        .collect::<Vec<_>>()
        .join(", ");
    let fixed = format!("listOf({values})");
    let Some(each) = option_each else {
        return fixed;
    };
    let item_context = context.with_item(&each.item, "row.value".to_string());
    let collection = escape_kotlin(&context.signal_path(&each.collection));
    let value = escape_kotlin(
        &item_context
            .item_path(&each.value)
            .unwrap_or_else(|| each.value.clone()),
    );
    let label = escape_kotlin(
        &item_context
            .item_path(&each.label)
            .unwrap_or_else(|| each.label.clone()),
    );
    let description = each
        ...
        .map(|path| {
            ...
            format!("state.text(\"{}\", row.value)", escape_kotlin(&path))
        })
        .unwrap_or_else(|| "null".to_string());
    let dynamic = format!(
        "state.rows(\"{collection}\").map {{ row -> DoweSelectOption(value = state.text(\"{value}\", row.value), label = state.text(\"{label}\", row.value), description = {description}) }}"
    );
    if options.is_empty() {
        dynamic
    } else {
