...
fn render_compose_overlay_item(
    ...
    let pad = " ".repeat(indent);
    let action = compose_optional_component_action(item.on_click.as_deref(), item.navigation.as_ref(), context);
    let action = close
        .map(|close| compose_closing_action(action.clone(), close))
        .unwrap_or(action);
    output.push_str(&format!(
...
fn compose_closing_action(action: String, close: &str) -> String {
    if action == "null" {
        return action;
    }
    let body = action
...
fn render_compose_button_spinner(icon: &SideNavIcon, indent: usize, output: &mut String) {
    let pad = " ".repeat(indent);
    output.push_str(&format!(
        "{pad}DoweSvg(viewBox = {}, modifier = {}, color = {}, paths = {}, animated = true)\n",
        compose_svg_view_box(&icon.props.view_box),
        modifier_for_style(&icon.props.style),
        compose_svg_color(&icon.props.style),
        compose_svg_paths(&icon.paths)
...
fn compose_fab_vertical_arrangement(position: OverlayCornerPosition) -> &'static str {
    match position {
        OverlayCornerPosition::TopLeft | OverlayCornerPosition::TopRight => "Alignment.Top",
