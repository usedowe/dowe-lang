fn register_compose_consumed_props(node: &ViewNode, context: &ComposeReactiveContext) {
    ...
    register_compose_structure_consumed_props(node, context);
    register_compose_media_consumed_props(node, context);
    register_compose_display_consumed_props(node, context);
    register_compose_control_consumed_props(node, context);
}
