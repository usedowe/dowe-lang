...
#[test]
fn generates_svg_aspect_ratio_for_single_compose_dimension() {
    ...
    assert!(
        views
            .content
            .contains(".aspectRatio(2.000000f, matchHeightConstraintsFirst = true)")
    );
}
...
fn generates_disabled_button_alpha_for_compose_and_launcher() {
    ...
    assert!(views.content.contains(
        "graphicsLayer { alpha = if (!(state.bool(\"formInvalid\", true))) 1f else 0.5f }"
    ));
    assert!(
        dev.content
...
#[test]
fn generates_android_percentage_widths() {
    let route = ViewRoute {
    ...
    assert!(views.content.contains("DoweSize.Percent(0.3f)"));
    assert!(views.content.contains("DoweSize.Percent(0.6f)"));
    assert!(
        views
            .content
            .contains("is DoweSize.Percent -> fillMaxWidth(value.fraction)")
    );
    assert!(
        views
            .content
            .contains("is DoweSize.Percent -> doweMinWidthFraction(value.fraction)")
    );
    assert!(dev.content.contains("dowePercentSize(30)"));
    assert!(dev.content.contains("dowePercentSize(60)"));
    assert!(dev.content.contains("doweApplyPercentWidth"));
    assert!(dev.content.contains("doweApplyPercentMinWidth"));
}
...
fn generates_android_viewport_minus_height() {
    ...
    assert!(
        views
            .content
            .contains("private fun doweViewportHeight(inset: Dp): Dp")
    );
    assert!(
        views
            .content
            .contains("(LocalConfiguration.current.screenHeightDp.dp - inset).coerceAtLeast(0.dp)")
    );
    assert!(dev.content.contains("doweViewportHeight(64)"));
    assert!(dev.content.contains("doweViewportHeight(80)"));
}

#[test]
fn keeps_bounded_dev_android_section_content_inside_section_padding() {
    let route = ViewRoute {
        id: "section-height".to_string(),
        route_path: "/section-height".to_string(),
        layout_tree: ViewNode::Children,
        page_tree: ViewNode::Section {
            props: StyleProps {
                sizing: dowe_components::SizingProps {
                    min_h: Some(ResponsiveValue::scalar(SizeValue::ViewportMinus(
