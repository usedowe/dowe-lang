fn android_runtime_avatar_chat() -> &'static str {
    r#"@Composable
private fun DoweAvatar(source: String?, name: String?, alt: String, size: String, status: String?, bordered: Boolean, backgroundColor: Color, contentColor: Color, borderColor: Color?, modifier: Modifier, onClick: (() -> Unit)?, hasIcon: Boolean, icon: @Composable () -> Unit) {
    ...
    LaunchedEffect(source) {
        bitmap = if (source.isNullOrBlank()) null else withContext(Dispatchers.IO) { doweLoadImageBitmap(context, source) }
    }
    Box(
        modifier = modifier
            .width(avatarSize)
...
private fun DoweAvatarGroup(items: List<DoweAvatarGroupItem>, size: String, maxCount: Int?, inline: Boolean, bordered: Boolean, backgroundColor: Color, contentColor: Color, borderColor: Color, modifier: Modifier) {
    ...
    Row(
        ...
        if (hiddenCount > 0) {
            Box(
                modifier = Modifier
                    .width(doweAvatarSize(size))
                    .height(doweAvatarSize(size))
                    .clip(RoundedCornerShape(999.dp))
                    .background(backgroundColor)
                    .border(if (bordered) 3.dp else 1.dp, borderColor, RoundedCornerShape(999.dp)),
                contentAlignment = Alignment.Center
            ) {
                Text(text = "+$hiddenCount", color = contentColor, fontSize = doweAvatarTextSize(size), fontWeight = FontWeight.SemiBold, maxLines = 1)
            }
        }
    }
}
    ...
    Column(
        ...
        if (showHeader) {
            Row(modifier = Modifier.fillMaxWidth().padding(bottom = 12.dp), verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.spacedBy(10.dp)) {
                DoweAvatar(source = assistantAvatar, name = assistantName, alt = assistantName, size = "sm", status = userStatus, bordered = false, backgroundColor = contentColor.copy(alpha = 0.08f), contentColor = contentColor, borderColor = contentColor, modifier = Modifier, onClick = null, hasIcon = false) {}
