...
fn render_pagination_html(
    props: &ToggleGroupProps,
    items: &[ToggleGroupItem],
    context: &ReactiveRenderContext,
) -> String {
    let selected = props.selected.parse::<usize>().unwrap_or(1);
    let rendered_pages = items.len().max(1);
    let initial_pages = props
        .pagination
        .as_ref()
        .map(|pagination| match pagination.total {
            dowe_components::PaginationTotal::Static(total) => {
                total.div_ceil(pagination.page_size).max(1) as usize
            }
    ...
    {
        extra.push_str(&format!(
            r#" data-dowe-pagination-total="{}" data-dowe-pagination-page-size="{}""#,
            escape_attr(&context.signal_path(total)),
    ...
    extra.push_str(&format!(
        r#" aria-label="{}""#,
        escape_attr(props.aria_label.as_deref().unwrap_or("Pagination"))
    ));
