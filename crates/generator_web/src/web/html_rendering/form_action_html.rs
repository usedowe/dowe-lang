...
fn render_theme_select_html(props: &ThemeSelectProps, context: &ReactiveRenderContext) -> String {
    let options = props
        .themes
        .iter()
        ...
        .collect::<Vec<_>>();
    let mut select_props = props.style.clone();
    select_props.label = Some(props.label.clone());
    select_props.placeholder = Some(props.placeholder.clone());
    render_select_html_with_attrs(
        &select_props,
        &options,
        None,
        context,
        &format!(
            r#" data-dowe-theme-select data-dowe-value="{}""#,
            escape_attr(&props.default_theme)
        ),
    )
}
...
fn render_fab_action_html(
    ...
    format!(
        r#"<div class="fab-action" style="--dowe-fab-action-delay:{}ms">{control}</div>"#,
        delay
    )
}
