    ...
    match item {
        ...
        NavMenuItem::Submenu { props, .. } | NavMenuItem::Megamenu { props, .. } => {
            format!(
                ...
                render_nav_menu_label_html(&props.label, props.i18n.as_deref()),
                render_nav_menu_arrow_html(context)
            )
        }
    }
}
...
fn render_nav_menu_subitem_html(
    ...
    let description = props
        .description
        .as_deref()
        .map(|value| localized_span("navmenu-submenu-description", value, props.description_i18n.as_deref()))
        .unwrap_or_default();
    format!(
        "<{tag}{attrs}>{}<span class=\"navmenu-submenu-content\">{}{description}</span></{close}>",
        render_nav_menu_subitem_icon_html(props.icon.as_ref(), context),
        render_nav_menu_subitem_label_html(&props.label, props.i18n.as_deref())
    )
}
...
fn render_nav_menu_arrow_html(context: &ReactiveRenderContext) -> String {
    let icon = solar_control_icon("alt-arrow-down").expect("bundled NavMenu arrow icon");
    render_svg_html(&icon.props, &icon.paths, context)
}

fn render_nav_menu_label_html(label: &str, i18n: Option<&str>) -> String {
    let i18n = i18n
        .map(|key| format!(r#" data-dowe-i18n="{}""#, escape_attr(key)))
        .unwrap_or_default();
    format!(
        r#"<span class="navmenu-label" data-text="{}"{i18n}>{}</span>"#,
        escape_attr(label),
        escape_html(label)
    )
}

fn render_nav_menu_subitem_label_html(label: &str, i18n: Option<&str>) -> String {
    let i18n = i18n
        .map(|key| format!(r#" data-dowe-i18n="{}""#, escape_attr(key)))
        .unwrap_or_default();
    format!(
        r#"<span class="navmenu-submenu-label"{i18n}>{}</span>"#,
        escape_html(label)
    )
}
...
fn render_scaffold_html(
    ...
    if !overlays.is_empty() {
        html.push_str("<div class=\"scaffold-overlays\">");
        html.push_str(&render_scaffold_region_html(overlays, children_html, context));
        html.push_str("</div>");
    }
    html.push_str("</div>");
    html
...
fn collect_scaffold_js_segments(
    ...
    if !overlays.is_empty() {
        push_literal(segments, "<div class=\"scaffold-overlays\">");
