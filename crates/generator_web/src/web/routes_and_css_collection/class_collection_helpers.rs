...
fn collect_rail_nav_icon_classes(items: &[RailNavItem], classes: &mut BTreeSet<String>) {
    for item in items {
        if let RailNavItem::Item(props) = item {
            classes.extend(svg_classes(&props.icon.props.style));
        }
    }
}
