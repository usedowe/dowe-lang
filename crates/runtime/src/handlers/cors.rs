...
fn insert_dynamic_header(response: &mut Response, name: &str, value: &str) {
...
fn append_header(response: &mut Response, name: &'static str, value: &str) {
    if let Ok(value) = HeaderValue::from_str(value) {
        response
            .headers_mut()
            .append(HeaderName::from_static(name), value);
    }
}
