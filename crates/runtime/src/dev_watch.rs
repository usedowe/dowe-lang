...
async fn handle_watch_changes(
    ...
    let compiled = compile_watch_project(root.to_path_buf()).await;
    ...
    match compiled {
        Ok(mut project) => {
            ...
            let server_init_action = selection
                .contains(DevTarget::Server)
...
async fn compile_watch_project(root: PathBuf) -> Result<Arc<CompiledProject>, String> {
    let (sender, receiver) = oneshot::channel();
    std::thread::Builder::new()
        .name("dowe-watch-compiler".to_string())
        .stack_size(16 * 1024 * 1024)
        .spawn(move || {
            let result = compile_dev(&root)
                .map(Arc::new)
