...
pub(crate) fn start_external_target(
    ...
    match target {
        ...
        DevTarget::Android => android::start(
            project,
            devices.android.clone(),
            devices.quit_simulators_on_exit,
            dev_origin,
        ),
        DevTarget::Ios => ios::start(
            project,
            devices.ios.clone(),
            devices.quit_simulators_on_exit,
            dev_origin,
...
pub(super) fn latest_child(path: PathBuf) -> RuntimeResult<PathBuf> {
    ...
    entries
        .into_iter()
        .max_by_key(|entry| child_version_key(entry))
        .ok_or_else(|| {
            RuntimeError::new(format!("missing tool directory under {}", path.display()))
        })
}

fn child_version_key(path: &std::path::Path) -> (bool, Vec<u64>, String) {
    let name = path
        .file_name()
    ...
    let stable = !name
        ...
        .any(|part| matches!(part, "alpha" | "beta" | "preview" | "rc"));
    let numbers = name
...
mod tests {
    use super::{
        StreamMode, cancel_active_external_commands, latest_child, quiet_command_options,
        run_required, target_started_message, target_starting_message,
    };
    ...
    #[test]
    fn latest_child_prefers_stable_numeric_tool_version() {
        let root = TempDir::new().expect("temp root");
        for version in ["35.0.0", "36.1.0-rc1", "36.1.0", "37.0.0-rc2"] {
            fs::create_dir(root.path().join(version)).expect("version directory");
        }

        let selected = latest_child(root.path().to_path_buf()).expect("selected version");
    ...
    #[test]
    fn latest_child_compares_numeric_platform_versions() {
        let root = TempDir::new().expect("temp root");
        for version in ["android-9", "android-36", "android-37"] {
            fs::create_dir(root.path().join(version)).expect("platform directory");
        }

        let selected = latest_child(root.path().to_path_buf()).expect("selected platform");
    ...
    #[test]
    fn latest_child_uses_newest_preview_when_no_stable_version_exists() {
        let root = TempDir::new().expect("temp root");
        for version in ["36.1.0-rc2", "37.0.0-rc1", "37.0.0-rc3"] {
            fs::create_dir(root.path().join(version)).expect("preview directory");
        }

        let selected = latest_child(root.path().to_path_buf()).expect("selected preview");
