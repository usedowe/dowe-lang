...
pub(super) fn prune_ios_app_cache(project_root: &Path, active_key: &str) -> RuntimeResult<()> {
    let cache_root = ios_cache_root(project_root);
    if !cache_root.is_dir() {
        return Ok(());
    }
    let mut inactive = fs::read_dir(&cache_root)?
        .filter_map(Result::ok)
        .filter(|entry| entry.path().is_dir())
        .filter(|entry| entry.file_name().to_string_lossy() != active_key)
        .collect::<Vec<_>>();
    inactive.sort_by_key(|entry| {
        entry
            .metadata()
    ...
    inactive.reverse();
    for entry in inactive.into_iter().skip(1) {
...
fn ios_host_input_files(root: &Path) -> RuntimeResult<Vec<PathBuf>> {
    let mut files = [
        root.join("dev/DoweIosDevHost.swift"),
...
#[cfg(test)]
mod tests {
    use super::{cached_ios_app, ios_app_cache_key, prune_ios_app_cache, publish_ios_app};
    use std::fs;
    ...
    fn ios_cache_key_changes_with_inputs_target_and_toolchain() {
        ...
        fs::create_dir_all(ios_root.join("dev")).expect("dev root");
        fs::write(ios_root.join("dev/DoweIosDevHost.swift"), "struct Host {}").expect("swift");
        fs::write(ios_root.join("GeneratedViews.swift"), "struct View {}").expect("views");
        ...
        let toolchain_changed =
            ios_app_cache_key(&ios_root, "arm64-apple-ios17.0-simulator", b"swift-2")
                .expect("cache key");
        fs::write(
            ios_root.join("GeneratedViews.swift"),
            "struct ChangedView {}",
        )
        .expect("views");
        let view_changed =
        ...
        let input_changed =
            ios_app_cache_key(&ios_root, "arm64-apple-ios17.0-simulator", b"swift-1")
                .expect("cache key");
        fs::create_dir_all(ios_root.join("Assets.xcassets/AppIcon.appiconset"))
            .expect("asset catalog");
        fs::write(
            ios_root.join("Assets.xcassets/AppIcon.appiconset/AppIcon.png"),
            "icon",
        )
        .expect("icon");
        let icon_changed =
    ...
    #[test]
    fn retains_active_and_one_previous_ios_host() {
        let temp = tempdir().expect("tempdir");
        ...
        assert!(cached_ios_app(temp.path(), "third").is_some());
        let retained = fs::read_dir(temp.path().join(".dowe/dev/ios/cache"))
            .expect("cache")
            .filter_map(Result::ok)
            .filter(|entry| entry.path().is_dir())
            .count();
