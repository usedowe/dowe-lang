...
use crate::engine::{init_database, kv_root};
...
const AUTH_MAGIC: &[u8] = b"DOWE_KV_AUTH_V1\n";

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct CreatedKvUser {
    pub database: String,
    pub user: String,
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct CreatedCacheAccount {
    pub name: String,
    pub account: String,
    pub secret: String,
    pub generated: bool,
    ...
    let (credential, generated) = match credential {
        ...
        Some(_) => {
            return Err(KvError::Authentication(
                "Cache secret must not be empty".to_string(),
            ));
        }
...
pub fn verify_user(
    ...
    if credential.is_empty() {
        return Err(KvError::Authentication(
            "Cache secret is required".to_string(),
        ));
    }
    ...
    for entry in users.iter().filter(|entry| entry.user == user) {
        ...
        if constant_time_eq(expected.as_bytes(), entry.token_hash.as_bytes()) {
            ...
            return Err(KvError::Authorization(format!(
                "Cache account `{user}` is not assigned to name `{database}`"
            )));
        }
    }
    let message = if saw_user {
        "Cache secret is invalid"
    } else {
        "Cache account is invalid"
    };
    Err(KvError::Authentication(message.to_string()))
}
...
pub fn create_account(
    ...
    account: &str,
    secret: Option<&str>,
) -> KvResult<CreatedCacheAccount> {
    let created = create_user(project_root, name, account, secret)?;
    Ok(CreatedCacheAccount {
...
pub fn verify_account(
    ...
    verify_user(project_root, name, account, secret)
}

fn read_users(project_root: &Path) -> KvResult<Vec<KvUser>> {
    ...
    if !reader.is_done() {
        return Err(KvError::Corruption(
            "KV auth catalog contains trailing bytes".to_string(),
        ));
    }
    Ok(users)
}

fn write_users(project_root: &Path, users: &[KvUser]) -> KvResult<()> {
...
fn auth_root(project_root: &Path) -> PathBuf {
    kv_root(project_root).join("_auth")
}
