use crate::{
    CacheProviderKind, CacheServerConfig, KvError, RemoteCacheClient, RemoteCacheConfig,
    clear_memory, create_account, init_database, list_databases, open_database, start_cache_server,
    verify_account,
};
use serde_json::json;
...
fn creates_and_verifies_database_users() {
    let temp = tempfile::tempdir().expect("temp");
    let created =
        create_account(temp.path(), "clinic", "clinic-api", Some("secret")).expect("account");

    assert_eq!(created.name, "clinic");
    assert_eq!(created.account, "clinic-api");
    ...
    verify_account(temp.path(), "clinic", "clinic-api", "secret").expect("verify");
    let error = verify_account(temp.path(), "other", "clinic-api", "secret").expect_err("authz");
    assert!(matches!(error, KvError::Authorization(_)));
}

#[tokio::test]
async fn dowe_cache_requires_auth_and_executes_operations() {
    let temp = tempfile::tempdir().expect("temp");
    create_account(temp.path(), "clinic", "clinic-api", Some("secret")).expect("account");
    ...
    let client = RemoteCacheClient::new(RemoteCacheConfig {
        provider: CacheProviderKind::Dowe,
    ...
    let bad_client = RemoteCacheClient::new(RemoteCacheConfig {
        provider: CacheProviderKind::Dowe,
        host: server.addr.ip().to_string(),
        port: server.addr.port(),
        account: "clinic-api".to_string(),
    ...
    crate::close_remote_connections();
    drop(client);
    drop(bad_client);
