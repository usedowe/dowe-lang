...
pub fn ulid_timestamp_millis(value: &str) -> Result<u64, IdError> {
    validate_ulid(value)?;
    let mut timestamp = 0u64;
    for byte in value.bytes().take(10) {
        timestamp = (timestamp << 5) | u64::from(decode_char(byte).expect("validated ulid"));
    }
    Ok(timestamp)
}
...
#[cfg(test)]
mod tests {
    use super::{generate_ulid, ulid_timestamp_millis, validate_ulid};
    ...
    #[test]
    fn reads_ulid_timestamp() {
        let id = generate_ulid();

        assert!(ulid_timestamp_millis(&id).unwrap() > 0);
    }
}
