...
pub fn web_target_dir(root: &Path, target: &str) -> DeployResult<PathBuf> {
    if target.is_empty()
        || target
            .chars()
            .any(|value| !(value.is_ascii_lowercase() || value == '-'))
    {
        return Err(DeployError::new("invalid web deploy target directory"));
    }
    Ok(root.join(".dowe/dist/web").join(target))
}
