...
fn init_project_harness_creates_project_agent_files() {
    ...
    let report = init_project_harness(temp.path(), InitOptions::default()).expect("init report");
...
fn init_agent_project_creates_root_instructions_and_harness() {
    ...
    assert!(root_agents.contains("only the direct imports"));
    assert!(root_agents.contains("Spec -> Contract -> Tests"));
    assert!(root_agents.contains("CodeGraph only when ownership"));
    ...
    let root_claude = fs::read_to_string(temp.path().join("CLAUDE.md")).expect("root claude");
...
fn harness_check_warns_when_project_agent_version_differs() {
    ...
    fs::write(&path, content.replace(env!("CARGO_PKG_VERSION"), "0.0.0")).expect("stale manifest");
...
fn update_managed_skills_preserves_project_skills() {
    ...
    init_agent_project_with_skills(temp.path(), InitOptions::default(), &initial_skills)
        .expect("init");
...
fn managed_skill_paths_reject_traversal_before_root_instructions_are_written() {
    ...
    let error = init_agent_project_with_skills(temp.path(), InitOptions::default(), &skills)
...
fn init_agent_project_preserves_and_explicitly_updates_root_instructions() {
    ...
    fs::write(temp.path().join("CLAUDE.md"), "user claude instructions").expect("root claude");
    ...
    assert!(updated.created.iter().any(|file| file.path == "AGENTS.md"));
}
...
fn init_agent_project_rejects_root_agents_symlink() {
    ...
    assert!(
        error
            .to_string()
            .contains("AGENTS.md must not be a symlink")
    );
    assert_eq!(fs::read_to_string(target).expect("outside"), "outside");
}
...
fn init_agent_project_rejects_root_claude_symlink() {
    ...
    assert!(
        error
            .to_string()
            .contains("CLAUDE.md must not be a symlink")
    );
    assert_eq!(fs::read_to_string(target).expect("outside"), "outside");
    assert!(!temp.path().join("AGENTS.md").exists());
    assert!(!temp.path().join(".agents").exists());
