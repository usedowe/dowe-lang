    ...
    fn validates_brand_children_navigation_and_size() {
        ...
        assert!(
            container_component_node(BuiltinComponent::Brand, Vec::new(), Vec::new(), false)
                .is_err()
        );
        assert!(
    ...
    fn resolves_solar_icon_styles_and_paints() {
        ...
        let duotone = icon_component_node(vec![
            string_prop("name", "alt-arrow-down"),
            string_prop("style", "bold-duotone"),
        ])
        .expect("duotone icon");
    ...
    fn exports_every_solar_variant_as_runtime_svg_data() {
        ...
        assert!(arrow.svg.starts_with("{\"viewBox\":\"0 0 24 24\",\"paths\":["));
        assert!(arrow.svg.contains("\"paint\":\"stroke\""));
    }

    #[test]
    fn resolves_svg_spinner_icons_and_rejects_non_linear_styles() {
        ...
        let ring = icon_component_node(vec![string_prop(
            "name",
    ...
    fn validates_side_nav_props_entries_and_icons() {
        ...
        assert!(invalid_scheme.to_string().contains("invalid value for prop `scheme`"));
    }
    ...
    fn validates_rail_nav_props_items_and_required_icons() {
        ...
        match node {
            ViewNode::RailNav { props, items } => {
                ...
                assert!(matches!(&items[0], super::RailNavItem::Item(props) if props.navigation.is_some()));
        ...
        assert!(missing_icon.to_string().contains("invalid value for prop `icon`"));
